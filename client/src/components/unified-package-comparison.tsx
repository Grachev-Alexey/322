...
export default function UnifiedPackageComparison({ 
  ...
  return (
    <div className="h-full flex flex-col" style={{ height: 'calc(100vh - 180px)' }}>
      {/* Package Headers */}
      <div className="grid grid-cols-4 gap-2 xl:gap-3 mb-2 xl:mb-3 flex-shrink-0">
        <div></div>
        {packageTypes.map((packageType) => {
          ...
          return (
            <div 
              key={packageType}
              className={`relative p-2 xl:p-3 rounded-lg border-2 transition-all duration-300 ${
                isSelected 
            ...
            >
              ...
              <div className="text-center">
                <div className={`w-8 h-8 xl:w-10 xl:h-10 mx-auto mb-2 bg-gradient-to-r ${info.gradient} rounded-lg flex items-center justify-center shadow-lg`}>
                  <Icon className="text-white" size={16} />
                </div>
                <h3 className="font-bold text-gray-900 dark:text-white text-sm xl:text-base">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 overflow-hidden flex-1 min-h-0">
          <div className="h-full overflow-y-auto">
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              ...
              return (
                <div key={perk.id} className={`grid grid-cols-4 gap-2 xl:gap-3 py-2 xl:py-3 px-3 xl:px-4 ${index % 2 === 0 ? 'bg-gray-50 dark:bg-gray-750' : 'bg-white dark:bg-gray-800'}`}>
                  {/* Perk Name */}
                  <div className="flex items-center space-x-2">
                    ...
                    <div className="min-w-0">
                      <div className="font-medium text-gray-900 dark:text-white text-sm xl:text-base">
                        {perk.name}
                      </div>
                    </div>
                  </div>
                          ...
                          <div className="text-center">
                            {perkValue.valueType === 'boolean' ? (
                              ...
                              <>
                                ...
                                {perkValue.isHighlighted && (
                                  <div className="flex items-center justify-center space-x-1 mt-1 hidden xl:flex">
      ...
      <div className="grid grid-cols-4 gap-2 xl:gap-3 mt-2 xl:mt-3 flex-shrink-0">
        <div></div>
        {packageTypes.map((packageType) => {
          const info = packageInfo[packageType];
          const packageData = packages.find((p: Package) => p.type === packageType);
