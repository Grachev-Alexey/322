...
export default function ThreeBlockComparison({
  ...
  return (
    <div className="space-y-2 lg:space-y-4 w-full max-w-4xl p-2 lg:p-4">
      ...
      >
        ...
        <div className="pt-3 lg:pt-4 px-4 lg:px-5 mb-3 lg:mb-6">
          {/* Title with star icon and package headers in grid */}
          <div className="grid grid-cols-4 gap-3 lg:gap-4 items-center p-1 lg:p-2">
            {/* Title with star icon */}
            <div className="flex items-center gap-2 lg:gap-3">
              <Star className="w-4 h-4 lg:w-5 lg:h-5 text-yellow-500" />
              <span className="font-bold text-gray-800 text-sm lg:text-lg">
                Преимущества
              </span>
            </div>
            ...
            {packageTypes.map((packageType) => {
              ...
              return (
                <div key={packageType} className="text-center">
                  ...
                  >
                    <div
                      className={`inline-flex items-center justify-center w-4 h-4 lg:w-5 lg:h-5 rounded-full bg-gradient-to-r ${getPackageColor(packageType)} mb-1`}
                    >
                      <Icon className="h-2.5 w-2.5 lg:h-3 lg:w-3 text-white" />
                    </div>
                    <div className="font-bold text-gray-800 text-xs lg:text-xs">
        ...
        <div className="px-4 lg:px-5 pb-3 lg:pb-5 relative z-0">
          <div className="space-y-1 lg:space-y-3">
            {uniquePerks.map((perk) => {
              ...
              return (
                <div
                  key={perk.id}
                  className="grid grid-cols-4 gap-3 lg:gap-4 py-1.5 lg:py-2 border-b border-gray-100 last:border-b-0"
                >
                  <div className="flex items-center text-xs lg:text-sm font-medium text-gray-700">
                    <span>{perk.name}</span>
                  </div>
                  {packageTypes.map((packageType) => {
            ...
            {/* Free Sessions Row */}
            <div className="grid grid-cols-4 gap-3 lg:gap-4 py-1.5 lg:py-2 border-b border-gray-100">
            ...
            {/* Discount Row */}
            <div className="grid grid-cols-4 gap-3 lg:gap-4 py-1.5 lg:py-2 border-b border-gray-100">
              <div className="text-xs lg:text-sm font-medium text-gray-700">Скидка</div>
              {packageTypes.map((packageType) => {
            ...
            {/* Bonus Account Row */}
            <div className="grid grid-cols-4 gap-3 lg:gap-4 py-1.5 lg:py-2 border-b border-gray-100 last:border-b-0">
              <div className="text-xs lg:text-sm font-medium text-gray-700">
                Бонусный счет
              </div>
              {packageTypes.map((packageType) => {
      ...
      >
        ...
        <div className="absolute -top-1 left-8 px-2 py-1 z-10">
          <div className="flex items-center gap-2 lg:gap-3">
            ...
            <span className="font-bold text-gray-800 text-sm lg:text-lg">Стоимость</span>
          </div>
        </div>

        <div className="pt-6 lg:pt-8 p-4 lg:p-6 space-y-2 lg:space-y-3">
          ...
          <div className="grid grid-cols-4 gap-3 lg:gap-4 py-1.5 lg:py-2 border-b border-gray-100">
            ...
            {packageTypes.map((packageType) => (
              <div key={packageType} className="text-center">
                <span className="text-xs lg:text-sm font-semibold text-red-500 line-through">
          ...
          <div className="grid grid-cols-4 gap-3 lg:gap-4 py-2 lg:py-3 mt-1 lg:mt-2">
            ...
            {packageTypes.map((packageType) => {
              ...
              return (
                <div key={packageType} className="text-center">
                  <span className="text-sm lg:text-base font-bold text-pink-400">
      ...
      <div className="space-y-1 lg:space-y-2 mt-2 lg:mt-4">
        {/* Первый взнос */}
        <div className="grid grid-cols-4 gap-3 lg:gap-4 py-1.5 lg:py-2">
          <div className="text-xs lg:text-sm text-gray-600">Первый взнос:</div>
          {packageTypes.map((packageType) => {
            return (
              <div key={packageType} className="text-center">
        ...
        {installmentMonths > 0 && (
          <div className="grid grid-cols-4 gap-3 lg:gap-4 py-1.5 lg:py-2">
            <div className="text-xs lg:text-sm text-gray-600">Платеж в месяц</div>
            {packageTypes.map((packageType) => {
              const packageData = getPackageData(packageType);
      ...
      >
        ...
        <div className="absolute -top-1 left-6 px-2 py-1 z-10">
          <div className="flex items-center gap-2 lg:gap-3">
            <span className="text-lg lg:text-xl">🎁</span>
            <span className="font-bold text-gray-800 text-sm lg:text-lg">Подарки</span>
          </div>
        </div>

        <div className="pt-6 lg:pt-8 p-4 lg:p-5 space-y-2 lg:space-y-3 relative z-0">
          ...
          {freeZones &&
            ...
            freeZones.map((zone, index) => (
              <div
                ...
                className="grid grid-cols-4 gap-3 lg:gap-4 py-1.5 lg:py-2 border-b border-gray-100"
              >
                <div className="text-xs lg:text-sm font-medium text-gray-700">
                  {zone.title}{" "}
